...
export default async function HostedPaySimulationPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <Link href={`/portal/orders/${payment.invoice.orderId}`} className="text-sm text-zinc-600 underline">
        ← Back to order
      </Link>
      <div>
        <h1 className="text-2xl font-semibold text-zinc-900">Hosted checkout</h1>
        {useAirwallexHpp ? (
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-4 text-sm">
        <p className="text-zinc-800">
          ...
          <span className="font-mono text-xs text-zinc-500">({payment.invoiceId})</span>
        </p>
        <p className="mt-2 text-xs text-zinc-500">Gateway: {payment.gateway}</p>
      </div>
